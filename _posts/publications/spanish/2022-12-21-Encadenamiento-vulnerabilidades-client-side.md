...
Al acceder al enlace, los parámetros proporcionados son reflejados directamente en la interfaz de usuario, lo cual indica la posible presencia de una vulnerabilidad de tipo XSS reflejado ([1], [2]). Se identificó que el parámetro ?nome= era vulnerable. El siguiente payload ilustra dicha explotación:
...
* El código vulnerable se encuentra en un subdominio, lo que impide el acceso a LocalStorage del dominio principal;
...
3. Redirección del usuario al payload utilizando un servicio de acortamiento de URLs: [https://cutt.ly/syPnJXp]()
...
La explotación encadenada de estas vulnerabilidades permite que un atacante provoque la “indisponibilidad” de la aplicación web para usuarios específicos. Esto se logra a través de la inyección masiva de cookies (cookie pollution), que supera los límites aceptables para el servidor, impidiendo que la aplicación sea cargada hasta que el usuario elimine manualmente las cookies maliciosas. Aunque este estudio se centró en un vector de impacto específico (DoS), podrían surgir otras posibilidades de explotación según configuraciones de la aplicación.
