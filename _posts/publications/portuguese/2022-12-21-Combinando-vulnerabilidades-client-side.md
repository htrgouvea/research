...
Durante uma análise na aplicação web da corretora easynvest.com.br (adquirida pelo Nubank), foram identificadas duas vulnerabilidades client-side que, quando exploradas em conjunto, podem comprometer a disponibilidade do serviço para usuários legítimos. Especificamente, observou-se a possibilidade de provocar uma condição de negação de serviço (DoS) a partir da combinação dessas falhas.
...
Ao acessar o link, os parâmetros fornecidos são refletidos diretamente na interface da aplicação, o que pode indicar a presença de uma vulnerabilidade do tipo XSS refletido ([1], [2]). Foi identificado que  o parâmetro `?nome=` estava vulnerável. O payload explorando essa característica foi:

- [https://indique.easynvest.com.br/?nome=<audio src/onerror=alert(1)>&id=1]()

![Imagem](/images/publications/nuinvest/xss-triaged.png)
￼
Apesar da existência da vulnerabilidade, sua exploração direta apresenta limitações relevantes:
...
* O domínio principal da aplicação conta com mecanismos primorosos de segurança, incluindo tokens anti-CSRF e cabeçalhos HTTP como `X-Frame-Options` (XFO), `Content Security Policy` (CSP) e `Cross-Origin Resource Sharing` (CORS) devidamente configurados.

Diante da limitação do XSS refletido isoladamente, buscou-se identificar outra vulnerabilidade que pudesse ser combinada para amplificar o impacto. A análise do código JavaScript da tela de autenticação revelou a presença de uma vulnerabilidade de Open Redirect ([3], [4]):
...
3. Encaminhamento do usuário via Open Redirect, utilizando um serviço de encurtamento de URLs para mascarar o payload: 
...
A cadeia de exploração permite que um atacante cause indisponibilidade localizada da aplicação web para usuários específicos. Isso ocorre por meio da injeção de um grande número de cookies (cookie pollution), que excede os limites esperados pelo servidor, impossibilitando o carregamento da aplicação até que os cookies sejam manualmente removidos pelo usuário. Embora este estudo tenha focado apenas em um vetor de impacto (DoS), outras possibilidades de exploração podem surgir a partir do mesmo vetor, dependendo das configurações da aplicação.

---

### Conclusão

A exploração demonstrada exige baixo esforço técnico e operacional, porém pode gerar impacto significativo ao usuário final. A utilização combinada de um XSS refletido e um Open Redirect viabiliza um ataque de negação de serviço via Cookie Bomb — um vetor pouco comum, mas eficaz em contextos específicos.
